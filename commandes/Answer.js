...
zokou({nomCom:"rahmany",reaction:"📡",categorie:"IA"},async(dest,zk,commandeOptions)=>{

  const {repondre,ms,arg}=commandeOptions;
  
    if(!arg || !arg[0])
    {return repondre("*ʜɪ ᴀᴍ 𝚁𝙰𝙷𝙼𝙰𝙽𝙸 𝙼𝙳, ʜᴏᴡ ᴍᴀʏ ɪ ʜᴇʟᴘ ʏᴏᴜ")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*powered by Rahmany*';
  ...
  zokou({ nomCom: "ai", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      const response = await axios.get(`https://lance-frank-asta.onrender.com/api/gpt?q=${question}`);
